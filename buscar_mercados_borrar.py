...
log=Logger('buscar_mercados_manual.log') 
logrsi=Logger('mercado_rsi_manual.log') 
logcompra=Logger('oportunidades_compra_manual.log') 
...
def vol_relativo(bb):
    return [  round(bb['vol'][0]/bb['ema'][0],2) , round(bb['vol'][1]/bb['ema'][1],2)     ]
    ...
    for p in precios:
        if p['symbol'].endswith('BTC') and not p['symbol'] in paresinutiles: # si la moneda es contra btc
            ...
            try:
                ...
                bb=i.volumen_bueno5(escala,1.5)# volumen por encima de su ema 
                if adx[0] >= 23 and macd[0]==1 and macd[1]<1 or bb['resultado'] : #adx con fuerza y macd con señal de compra
                    lista_oportunidades.append([p['symbol'],p['price'] , rsi , adx, vol_relativo(bb)])
