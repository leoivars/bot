...
def probar_mp_slice(par,horas):
    ind=Indicadores(par,log,e,client)
    prop=Par_Propiedades(par,client,log)
    result = ind.mp_slice(horas,prop.tickSize)
...
def probar_ind(cvelas=None):
    ind=Indicadores('BTCUSDT',log,e,client)
    ind.actualizar_velas('1d')
    print ('high',ind.velas['1d'].valores_np_high(cvelas))
    print ('low',ind.velas['1d'].valores_np_low(cvelas))
    print ('open',ind.velas['1d'].valores_np_open(cvelas))
    print ('close',ind.velas['1d'].valores_np_close(cvelas))
    print ('volume',ind.velas['1d'].valores_np_close(cvelas))
    print ('df',ind.velas['1d'].panda_df(cvelas))

def probar_ind_df(cvelas=None):
    ind=Indicadores('BTCUSDT',log,e,client)
    ind.actualizar_velas('1d')

    df = ind.velas['1d'].panda_df(cvelas)
    print ('df',df,df._is_view)    

def mostrar_emar_emal(par,escala,r,l):
    ind=Indicadores(par,log,e,client)
    r = ind.ema_vector_completo(escala,r)
    l = ind.ema_vector_completo(escala,l)
    p = ind.precio_vector_completo(escala)
...
def probar_vp(par,escala):
    ind=Indicadores(par,log,e,client)
    r = ind.vp(escala,24)
    o =r.sort_values(by ='total_volume' ,ascending=False) 
    c = o["mean_close"]
    print (o,c)
    for d in c:
        print(d,type(d) )

    print(o["mean_close"][0])    
    #matplotlib.use('TkAgg')    
    #fig, ax = plt.subplots()
    #fig, hi = plt.subplots()
    #ax.plot(r [   [ 'low_close',  'mean_close' , 'high_close' ]        ],'-r' )
    #hi.hist(r [   [ 'total_volume' ]        ], orientation='horizontal')
    #plt.show()



probar_vp('AVAXUSDT','1h')
