...
from estrategia_squeeze_01 import Estrategia
...
if __name__=='__main__':
    ...
    while m.fecha_fin < fin_test:
        ...
        print( f'                                                 {txtf}-------> ganancia= {gananciap:.2f} entradas {entradas} trades {len(trades)} {trades} '  )
        ...
        if not comprado:
            ...
            recomprado = False
            gan = g.calculo_ganancia_porcentual(comp_px,px_vta)
            log.log (f'{txtf} -comprado-> {gan:.2f}%  sl {stoploss} ')    
            
            if gan <0 and  estrategia.decision_recompra(comp_px):
                log.log (f'-evento- recompra gan {gan:.2f} entradas {entradas}')
                recomprado = True
                comprado = False
            ...
            if not recomprado and not vendido and stoploss>0:
                ...
                if  stoploss >0 and stoploss < sl < px_vta:
                    ...
                    log.log (f'-evento- stoploss_subido {stoploss} a {sl} gan {gan:.2f}  ')
                    stoploss = sl

            if vendido:
                log.log (f'-evento- venta  gan {gan:.2f} entradas {entradas}')
                gananciap += gan
                if gan >0:
                    signo='+'
                    ganadas+=1
                ...
                log.log (f'----->Termina operacion {signo} gan {gan:.2f} entradas {entradas}') 
                comprado=False
    ...
    if comprado:   #guardo compra abierta    
        ...
        if gan >0:
            ...
            perdidas+=1    
        entradas+=1
        log.log (f'----->Termina operacion gan {gan:.2f} entradas {entradas}') 
        comprado=False

    txt_log_paremtros = f'{descripcion_periodo}: escala={estrategia.escala_rapida}  fechas {fecha_fin}-{fin_test}'
    txt_log_datos     = f'{par} ganancia= {gananciap:.2f} entradas {entradas} ganadas {ganadas} perdidas {perdidas} trades {len(trades)} max_trades {max_trades} '  
