...
class Estrategia():
    ...
    def decision_de_compra(self):
        ...
        if self.filtro.pendiente_positiva_ema(self.escala_rapida,55):
            ...
            if self.filtro.rsi_maximo(self.escala_rapida,45):
                if self.filtro.rango_de_compra(self.escala_rapida,-20,0.5):               # para no comprar en la parte alta
                    if self.filtro.squeeze_negativo_sqz_off(self.escala_rapida):
                        return True
    ...
    def stoploss(self,precio_compra):
        ...
        minimo,maximo=self.filtro.rango.min_max(self.escala_rapida)
        gan_protecion = self.g.calculo_ganancia_porcentual(precio,   precio+(maximo-minimo)/2  )
        self.log.log(f'stoploss  gan_minimo {gan_stoploss}  gan_protecion {gan_protecion}   ')
        if  gan_stoploss>0:
