...
class Estrategia():
    ...
    def decision_de_compra(self,escala,ultima_compra=None):
        ...
        if ultima_compra:
            ...
            gan = self.g.calculo_ganancia_porcentual(ultima_compra,precio)
            gan_lim=self.__gan_limite__(escala)
            if gan > gan_lim:
                self.log.log(f'gan {gan} gan_lim {gan_lim} ultima_compra {ultima_compra} precio {precio}')
                return False
            
        if self.filtro.rango_de_compra(escala,minimo_compra=-5,maximo_compra=0.23):
            ret = True
            
            
            
            # if self.filtro.cruce_emas_hacia_arriba(escala,9,21,5):
            #     ret = True
            
            # if not ret:
            #     if self.filtro.rsi_minimo_cercano(escala,20,(2,5),35):
            #         ret = True
        return ret    
    ...
    def __gan_limite__(self,escala):
    ...
    def decision_recompra(self,escala,precio_compra):
        ...
        ret = False
        if gan < self.__gan_limite__(escala):
            ret = True
